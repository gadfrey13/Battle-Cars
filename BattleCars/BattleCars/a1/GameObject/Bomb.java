...
public class Bomb {
	...
	public Bomb(SceneManager sm,MyPhysicsEngine physicsEngine,int num,SceneNode noe) throws IOException{
		UUID ID = UUID.randomUUID();
		missile = sm.createEntity("missile" +num+ID+"bomb", "missileunwrapfix.obj");
		missile.setPrimitive(Primitive.TRIANGLES);
		node = sm.getRootSceneNode().createChildSceneNode("missile"+num+ID+"bomb");
		...
		node.setLocalPosition(noe.getLocalPosition());
	...
	public Bomb(Player object,SceneManager sm,MyPhysicsEngine physicsEngine,int num,String enemy,IAudioManager audioMgr) throws IOException{
		UUID ID = UUID.randomUUID();
